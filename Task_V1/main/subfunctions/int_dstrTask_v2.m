...
% send trigger
int_triggerSend(cfg.trigger,'distract_start')
...
cfg.dstr.col.dgrey = greyvalues(gradingIndex,1).*255; %0.3175*255
cfg.dstr.col.lgrey = greyvalues(length(greyvalues)-gradingIndex,1).*255; %0.6667*255
cfg.dstr.col.white = greyvalues(length(greyvalues)-gradingIndex,1).*255; %so there is no light gray only dark
...
while x+cfg.time.distract > GetSecs
    ...
    if rand(1) > 0.5; 
        trl_col = cfg.dstr.col.white; et = 'lure';
    else; 
        trl_col = cfg.dstr.col.dgrey; count = count + 1; et = 'target';
    end
...
WaitSecs(1);
% send trigger
int_triggerSend(cfg.trigger,'distract_end')
...
% wait for response
while true
        RestrictKeysForKbCheck([38 40 32]);
    resp = 0;
...
%     [resp,t2] = int_getResponse(cfg);
    ...
    elseif respo == 40
        ...
        % update cursor position
        if key_value ~= 0 ; key_value = key_value - 1;
        else; key_value = 10 ;
        end
...
DrawFormattedText(cfg.screen.w1,'How many dark grey crosses were presented?\n\nLeft Button to increase value. Right button to confirm',cfg.screen.pos.confiQ(1)-round(x(3)./2),cfg.screen.pos.confiQ(2)-round(x(4)./2),[50 50 50]); % draw texture

[~,~,x] = DrawFormattedText(cfg.screen.w1,sprintf('[ %d ]',key_value),[],[],[0 0 0]);
DrawFormattedText(cfg.screen.w1,sprintf('[ %d ]',key_value),cfg.screen.pos.confiR2(1)-round(x(3)./2),cfg.screen.pos.confiR2(2)-round(x(4)./2),[50 50 50]); % draw texture
